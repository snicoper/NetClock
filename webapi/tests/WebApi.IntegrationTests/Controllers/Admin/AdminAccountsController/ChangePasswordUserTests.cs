...
{
    public class ChangePasswordUserTests : BaseControllerTest
    {
        private readonly UserManager<ApplicationUser> _userManager;

        public ChangePasswordUserTests(CustomWebApplicationFactory<Startup> factory) : base(factory)
        {
            ...
            string confirmPassword)
        {
            ...
            var user = await _userManager.FindByNameAsync("Bob");
            var userId = user.Id;
            var data = new ChangePasswordUserCommand(userId, newPassword, confirmPassword);
            var requestContent = SerializerUtils.GetRequestContent(data);
            var url = Utilities.ComposeUri("admin/accounts/change-password");
